...
def request(method, path, key, payload=None):
    ...
    if payload and "_image_b64" in payload:
        boundary = "api429-" + uuid.uuid4().hex
        parts = []
        ...
        parts.append(base64.b64decode(payload["_image_b64"]))
        parts.append(f"\r\n--{boundary}--\r\n".encode())
        body = b"".join(parts)
    ...
    try:
        with urllib.request.build_opener(NoRedirect).open(req, timeout=180) as response:
            ...
            if len(raw) > MAX_RESPONSE:
                raise API429Error(
                    "Response too large; reconcile request before retrying."
                )
            return response.status, dict(response.headers), json.loads(raw)
    except urllib.error.HTTPError as exc:
        raise API429Error(
            f"API429 HTTP {exc.code}. No generation retry was sent."
        ) from None
    except (OSError, ValueError):
        raise API429Error(
            "Response unavailable or invalid. Do not create a new run ID until the original request is reconciled."
        ) from None


class Client:
    def __init__(
        self, key, directory, transport=request, sleep=time.sleep, clock=time.monotonic
    ):
    ...
    def generate(
        ...
        if endpoint not in ("/v1/images/generations", "/v1/images/edits"):
            raise API429Error("Unsupported endpoint")
        if not self.key:
            raise API429Error("Set API429_API_KEY in the ComfyUI server environment.")
        if not re.fullmatch(r"[a-zA-Z0-9_-]{1,80}", run_id):
            raise API429Error(
                "Run ID must contain 1–80 letters, digits, hyphens or underscores."
            )
        identity = (
            payload
            if endpoint == "/v1/images/generations"
            else {"endpoint": endpoint, "payload": payload}
        )
        ...
        try:
            ...
            raise API429Error(
                "This run is active or was interrupted. Reconcile it before removing its lock."
            ) from None
        os.close(lock)
        try:
            ...
            if record_path.exists():
                ...
                if record["digest"] != digest:
                    raise API429Error(
                        "Run ID already belongs to different parameters. Use a new ID only for an intentional new generation."
                    )
                if record["state"] == "complete":
                    return record["response"]
                if record["state"] != "pending":
                    raise API429Error(
                        "Previous POST outcome is unknown. Check API429 history/support before starting another run."
                    )
            else:
                if not approved:
                    raise API429Error(
                        "Enable allow_paid_generation to submit one paid image request."
                    )
                record = {"digest": digest, "state": "submitted"}
                save(record)
                status, headers, data = self.transport(
                    "POST", endpoint, self.key, payload
                )
                if status == 200:
                ...
                if status != 202:
                    raise API429Error(
                        "Unexpected status. Original request retained; no automatic retry."
                    )
                ...
                if not isinstance(path, str) or not re.fullmatch(
                    r"/v1/balancer/jobs/[a-zA-Z0-9_-]+/result", path
                ):
                    record.update(response=data)
                    save(record)
                    raise API429Error(
                        "Unrecognized async response; original receipt retained for reconciliation."
                    )
            ...
            while self.clock() < end:
                status, headers, data = self.transport(
                    "GET", record["result_path"], self.key
                )
                if status == 200:
                    record.update(state="complete", response=data)
                    save(record)
                ...
                if status != 202:
                    raise API429Error(
                        "Unexpected polling status; rerun this same ID to resume without another POST."
                    )
                try:
                ...
                self.sleep(delay)
            raise API429Error(
                "Still pending. Rerun with the same run ID and parameters to resume polling without another charge request."
            )
        finally:
